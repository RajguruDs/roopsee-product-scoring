...
def generated_at() -> str:
    """Actual build date, overridable for reproducible builds."""
    return os.environ.get("ROOPSEE_GENERATED_AT") or datetime.date.today().isoformat()


def display_path(path: Path) -> str:
    # Always POSIX separators: this string ships inside the dataset and should
    # not change shape depending on which OS produced the build.
...
onboarding = load_module(REPO_ROOT / "tools" / "onboarding.py", "roopsee_onboarding")
canonical_population = load_module(REPO_ROOT / "tools" / "canonical_population.py", "roopsee_canonical_population")

ONBOARD_LIMIT = onboarding.ONBOARD_LIMIT
CANONICAL_OUTPUT_DIR = env_path("ROOPSEE_CANONICAL_OUTPUT_DIR", REPO_ROOT / "outputs" / "roopsee_canonical")
SCORED_POPULATION_PATH = CANONICAL_OUTPUT_DIR / "scored_population_full.json"
ONBOARDING_STATUS_PATH = CANONICAL_OUTPUT_DIR / "onboarding_status.csv"
MIGRATION_AUDIT_PATH = CANONICAL_OUTPUT_DIR / "migration_audit.json"
MIGRATION_AUDIT_MARKDOWN = CANONICAL_OUTPUT_DIR / "migration_audit.md"
...
def write_population_outputs(
    ...
    """Write the full scored population, the per-product status roster, and the audit.

    These are the master artefacts. The frontend dataset is a curated subset of
    them, so the full population stays available whatever ONBOARD_LIMIT is set to.
    """
    CANONICAL_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    population = [
        {**{key: value for key, value in product.items() if key not in ("_evidenceQuality", "_broadlySafe")},
         "eligibilityStatus": statuses.get(product["uid"], (onboarding.STATUS_ELIGIBLE, ""))[0],
         "eligibilityReason": statuses.get(product["uid"], (onboarding.STATUS_ELIGIBLE, ""))[1],
         "onboarded": product["uid"] in onboarded_uids,
         "evidenceQuality": product.get("_evidenceQuality"),
         }
        for product in scored_products
    ]
    SCORED_POPULATION_PATH.write_text(
        json.dumps(
            {
                "metadata": {
                    "generatedAt": generated_at(),
                    "populationSource": os.environ.get("ROOPSEE_POPULATION_SOURCE", "canonical"),
                    "scoredProductCount": len(population),
                    "onboardLimit": ONBOARD_LIMIT,
                    "onboardedCount": len(onboarded_uids),
    ...
    onboarding.write_status_csv(scored_products, statuses, onboarded_uids, ONBOARDING_STATUS_PATH)
    MIGRATION_AUDIT_PATH.write_text(json.dumps(audit, indent=2, ensure_ascii=False), encoding="utf-8")
    MIGRATION_AUDIT_MARKDOWN.write_text(onboarding.audit_markdown(audit), encoding="utf-8")
...
def build_dataset() -> dict[str, Any]:
    ...
    for index, record in enumerate(large_records, start=1):
        ...
        products.append(
            {
                "uid": record.uid,
                "gtin": clean(record.row.get("gtin")),
    ...
    # Every scored product is kept intact before any gate is applied, so the
    # full scored population can be written and excluded products stay auditable.
    scored_products = products

    # Eligibility and onboarding are separate from scoring: nothing below here
    # changes a score, it only decides which scored products the frontend ships.
    for product in scored_products:
        product["_evidenceQuality"] = onboarding.evidence_quality(product, SCORE_COLUMNS, VISIBLE_SCORE_WEIGHTS)
        product["_broadlySafe"] = onboarding.is_broadly_safe(product, SCORE_COLUMNS, VISIBLE_SCORE_WEIGHTS)

    # Only the canonical population is GTIN-keyed; the legacy one has no GTINs
    # at all, so requiring one there would exclude every product.
    require_gtin = os.environ.get("ROOPSEE_POPULATION_SOURCE", "canonical").strip().lower() != "legacy"
    seen_gtins: set[str] = set()
    statuses: dict[str, tuple[str, str]] = {}
    for product in scored_products:
        statuses[product["uid"]] = onboarding.evaluate_eligibility(
            product,
            SCORE_COLUMNS,
            VISIBLE_SCORE_WEIGHTS,
            seen_gtins=seen_gtins,
            is_non_topical=is_non_topical_catalog_product,
            validate_gtin=canonical_population.validate_gtin,
            require_gtin=require_gtin,
        )

    eligible = [
        product
        for product in scored_products
        if statuses[product["uid"]][0] == onboarding.STATUS_ELIGIBLE
    ]
    products = onboarding.select_onboarding(eligible, ONBOARD_LIMIT)

    excluded_non_topical_count = sum(
        1 for status, _reason in statuses.values() if status == onboarding.STATUS_NON_TOPICAL
    )
    high_confidence_before_topical_filter = len(eligible) + excluded_non_topical_count
    ...
    onboarded_uids = {product["uid"] for product in products}
    audit = onboarding.build_audit(
        scored_products,
        statuses,
        products,
        canonical_count=all_product_count,
        limit=ONBOARD_LIMIT,
        extra={"populationSource": os.environ.get("ROOPSEE_POPULATION_SOURCE", "canonical")},
    )
    write_population_outputs(scored_products, statuses, onboarded_uids, audit)

    # The internal ranking helpers never ship to the browser.
    for product in scored_products:
        product.pop("_evidenceQuality", None)
        product.pop("_broadlySafe", None)
    ...
    return {
        "metadata": {
            ...
            "scoredProductCount": all_product_count,
            ...
            "selectionRule": (
                "All scored products pass eligibility; the onboarding population is "
                f"type-balanced by evidence strength and capped at "
                f"{ONBOARD_LIMIT if ONBOARD_LIMIT is not None else 'no limit'}"
            ),
            "eligibilityStatusCounts": audit["statuses"],
            "doctorReferenceProductCount": len(doctor_records),
            "sourceFiles": {
                "canonicalPopulation": display_path(canonical_population.CANONICAL_POPULATION),
                "ingredientMapping": display_path(canonical_population.INGREDIENT_MAPPING),
                ...
                "doctorReference": "data/products.csv",
            },
            "visibleScoreWeights": VISIBLE_SCORE_WEIGHTS,
            "rankFusionWeights": RANK_FUSION_WEIGHTS,
            "validation": {
