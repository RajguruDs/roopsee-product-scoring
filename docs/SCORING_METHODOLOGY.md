...
The platform runs on the canonical v2 product population, keyed by GTIN.

Two populations must be kept distinct:

- **Scoring population: 4,037.** Every canonical product is scored.
- **Onboarding population: 1,000.** That subset is what the frontend ships.

How the numbers reach the UI:

- Canonical products scored: 4,037
- High-confidence products: 3,875 (96%)
- Eligible after quality, safety and topical checks: 3,841
- Onboarded to the frontend: 1,000 (`ONBOARD_LIMIT`, configurable)
- Doctor-reference products: 384 (unchanged)
...
Onboarded product type coverage:
...
Onboarding is type-balanced in proportion to the eligible mix, so every routine slot
stays well covered. These numbers come from the generated frontend dataset and change
whenever the source sheets or `ONBOARD_LIMIT` change.

## 3. Source Files Used

The scoring system uses these source files:

- `data/source/canonical_scoring_population_v2.csv`: the product population. One canonical product per GTIN, with brand, product name, category, price, URL, image, full INCI, and key ingredients.
- `data/source/scoring_input_ingredient_mapping_v2.csv`: the ingredient intelligence layer, joined 1:1 to the population on `canonical_product_id_v2`. It carries ingredients already resolved against the score master, the mapped product type, and mapping confidence. It is not a second product population and is never scored independently.
...
Legacy inputs, retained and still runnable via `ROOPSEE_POPULATION_SOURCE=legacy`:

- `data/source/useful_skin_bodycare_products.xlsx`: the previous name-keyed population.
- `data/source/retailer_products_rows.csv.gz`: retailer export the legacy path joined against by product name.
...
## 3a. Primary And Secondary Ingredient Selection

Primary ingredients are chosen from evidence, not from INCI order and not from the mere
fact that an ingredient matched the score master. The ladder, strongest first — the
first tier that yields anything wins:

1. **Product name / explicit hero claim.** Percentage-qualified actives ("10% Niacinamide")
   and any score-master ingredient or curated alias named in the title, including
   parenthetical synonyms ("Thiamidol"). A product may claim several heroes; all are kept.
2. **`key_ingredients`** resolved against the score master.
3. **Resolved score-master ingredients**, covering alias-only resolutions.
4. **Full INCI matches**, which also flags the product for review.

A name claim only counts when the ingredient data corroborates it.

Secondary ingredients are meaningful supporting ingredients. Generic formulation
ingredients — solvents, preservatives, chelators, pH adjusters, generic thickeners and
emulsifiers — are never promoted. Fragrance, glycerin, cetearyl alcohol and squalane are
deliberately kept, because Roopsee scores them as real actives.

Primary is capped at 4 ingredients and secondary at 12, so a long INCI cannot dilute the
product-type weighting described in section 8.
...
- No toner in the current catalog is suitable for an Excessive Dryness profile. Every eligible toner is hard-blocked by the calibrated layer, because the ternarisation in section 10 maps any predicted value at or below 50 to `-100` and the catalog's toners are actives-led. The doctor data is less absolute: 11 of 25 doctor-scored toners are `-100` for excessive dryness. Toner is not a routine slot, so routines are unaffected, but the toner shelf will be empty for that profile.
- The onboarding selection reserves part of each product type's quota for products that are not hard-blocked for any special condition. Without that reserve, ranking by evidence alone fills the catalog with actives-led products and leaves risk profiles with nothing to recommend.
...
| `docs/MIGRATION_CANONICAL_V2.md` | What the canonical migration changed and deliberately did not change. |
| `data/source/canonical_scoring_population_v2.csv` | Canonical product population (primary input). |
| `data/source/scoring_input_ingredient_mapping_v2.csv` | Ingredient mapping layer joined to the population. |
| `data/source/useful_skin_bodycare_products.xlsx` | Legacy product ingredient source sheet. |
| `data/source/retailer_products_rows.csv.gz` | Compressed retailer catalog metadata source (legacy path). |
| `data/source/roopsee_ingredient_scores_v3.xlsx` | Ingredient score master. |
| `data/products.csv` | Doctor-reference products used as scoring anchors. |
| `data/Product details and score logic.xlsx` | Doctor-reviewed reference workbook. |
| `tools/canonical_population.py` | Loads the canonical population and selects primary/secondary ingredients. |
| `tools/onboarding.py` | Eligibility, onboarding selection, and the migration audit. |
