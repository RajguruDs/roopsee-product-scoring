...
# Which product population to score. "canonical" is the GTIN-keyed v2 population;
# "legacy" is the original name-keyed useful_skin_bodycare_products.xlsx, kept
# runnable so the previous dataset can still be reproduced and diffed.
POPULATION_SOURCE = os.environ.get("ROOPSEE_POPULATION_SOURCE", "canonical").strip().lower()
...
def full_inci_from_retailer(row: dict[str, Any] | None) -> tuple[str, bool]:
    ...
    if not ingredients:
        return "", False
    # A source that guarantees the column is a complete INCI list can say so
    # directly instead of relying on the scraped "full ingredient list:" marker.
    if row.get("is_full_inci"):
...
def split_ingredient_parts(text: str, *, split_plus: bool = False) -> list[str]:
    """Split an ingredient list on real delimiters only.

    Two kinds of comma are not delimiters and must survive intact:
    a comma between two digits is a chemical locant (1,2-Hexanediol,
    1,3-Butylene Glycol), and a comma inside brackets belongs to the
    ingredient name it qualifies. Slashes are never delimiters, so
    Caprylic/Capric Triglyceride stays whole.
    """
    parts: list[str] = []
    buffer: list[str] = []
    depth = 0
    ...
    for index, char in enumerate(text):
        ...
        if depth == 0:
            ...
            if char == "," and not (previous.isdigit() and following.isdigit()):
                parts.append("".join(buffer))
                buffer = []
                continue
            if split_plus and char == "+" and previous.isspace() and following.isspace():
                parts.append("".join(buffer))
                buffer = []
                continue
...
def split_full_inci_items(inci_text: str) -> list[str]:
    return [clean_text(part) for part in split_ingredient_parts(inci_text) if clean_text(part)]
...
def split_ingredients(value: str) -> list[str]:
    ...
    parts = split_ingredient_parts(text, split_plus=True)
    output: list[str] = []
...
def load_legacy_population() -> list[dict[str, Any]]:
    """Legacy population: xlsx rows joined to the retailer scrape by product name."""
    useful_products = read_useful_products()
    retailer_by_name = read_retailer_rows()
    for product in useful_products:
        rows = retailer_by_name.get(product["product_name_key"], [])
        product["retailer_rows"] = rows
        product["representative"] = best_retailer_row(rows)
    return useful_products


def load_canonical_population(alias_map: dict[str, str]) -> list[dict[str, Any]]:
    """Canonical v2 population: one GTIN-keyed product per row, already mapped."""
    # Loaded by path rather than by name: these tools are standalone scripts,
    # not a package, so a plain import depends on how the caller was launched.
    spec = importlib.util.spec_from_file_location(
        "roopsee_canonical_population", Path(__file__).resolve().parent / "canonical_population.py"
    )
    if spec is None or spec.loader is None:
        raise SystemExit("Could not load tools/canonical_population.py")
    canonical_population = importlib.util.module_from_spec(spec)
    # Register before executing: dataclasses resolve their annotations through
    # sys.modules, so the module must be findable by name while it is running.
    sys.modules[spec.name] = canonical_population
    spec.loader.exec_module(canonical_population)

    products = canonical_population.load_canonical_products(alias_map)

    # Write the scoring input as part of the normal pipeline run, not only when
    # canonical_population.py is invoked on its own. Downstream tooling (the
    # validation framework, audits) expects it to exist after a plain rebuild.
    scoring_input_path = canonical_population.write_scoring_input(products)
    print(f"Scoring input: {len(products)} products -> {scoring_input_path}")

    population: list[dict[str, Any]] = []
    for product_dict, representative in canonical_population.as_population_rows(products):
        product_dict["uid"] = product_dict["canonical_id"]
        product_dict["representative"] = representative
        product_dict["retailer_rows"] = [representative]
        population.append(product_dict)
    return population


def load_population(alias_map: dict[str, str]) -> list[dict[str, Any]]:
    if POPULATION_SOURCE == "legacy":
        return load_legacy_population()
    if POPULATION_SOURCE == "canonical":
        return load_canonical_population(alias_map)
    raise SystemExit(
        f"Unknown ROOPSEE_POPULATION_SOURCE={POPULATION_SOURCE!r}; expected 'canonical' or 'legacy'."
    )


def build_rows() -> dict[str, Any]:
    canonical_rows, alias_map, alias_labels, original_ingredient_rows = read_ingredient_scores()
    useful_products = load_population(alias_map)
    print(f"Population source: {POPULATION_SOURCE} ({len(useful_products)} products)")
    ...
    for product_index, product in enumerate(useful_products, start=1):
        retailer_rows = product.get("retailer_rows", [])
        representative = product.get("representative") or best_retailer_row(retailer_rows)
        # The canonical population supplies a mapped product type; the legacy
        # population has none and falls back to inferring it from the name.
        product_type = product.get("product_type") or product_type_from(product["product_name"], representative)
        ...
        auto_uid_base = product.get("uid") or f"AUTO-{product_index:05d}"
        score_basis = (
    ...
    matched_product_names = sum(1 for product in useful_products if product.get("retailer_rows"))
    matched_sku_rows = sum(len(product.get("retailer_rows") or []) for product in useful_products)
    unmatched_count = total_useful_names - matched_product_names
    summary_rows = [
        ["Metric", "Value"],
        ["Population source", POPULATION_SOURCE],
        ["Products scored", total_useful_names],
        ["Products matched to retailer metadata", matched_product_names],
        ["Products missing retailer metadata", unmatched_count],
        ...
        [
            "Primary source",
            str(USEFUL_PRODUCTS) if POPULATION_SOURCE == "legacy" else "data/source/canonical_scoring_population_v2.csv",
        ],
        ["Retailer metadata CSV", str(RETAILER_PRODUCTS) if POPULATION_SOURCE == "legacy" else "n/a (canonical)"],
