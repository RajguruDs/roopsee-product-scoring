// LOCALHOST ONLY. `?scorer=v3` runs the Experimental V3 engine served by app.py.
// Without the flag every line below behaves exactly as it always has: the shipped
// dataset is loaded and scored by the production JavaScript scorer in this file.
// Fail-safe: any non-browser host (the Node scoring harness, a test sandbox) has no
// window.location, and must fall through to the production scorer rather than throw.
const V3_MODE = (() => {
  try {
    return new URLSearchParams(window.location.search).get("scorer") === "v3";
  } catch (error) {
    return false;
  }
})();
const DATA_URL = V3_MODE ? "/api/v3/dataset" : "./data/final_scored_products.json";
const V3_SCORE_URL = "/api/v3/score";
let v3ScoreByUid = null;
let v3Meta = null;
...
async function fetchV3Scores() {
  const response = await fetch(V3_SCORE_URL, {
    method: "POST",
  ...
  if (!payload.ok) throw new Error(payload.error || "V3 scoring failed");
  v3ScoreByUid = new Map(payload.rows.map((row) => [row.uid, row]));
  v3Meta = payload;
  console.info(
    `[V3] scored ${payload.productsScored} products, ${payload.failures.length} failure(s), ` +
      `${payload.fallbacks} fallback(s), range ${payload.scoreRange?.min}-${payload.scoreRange?.max}, ` +
      `ranking: ${payload.ranking}`
  );
}

// Synchronous by design: fetchV3Scores() has already populated the map, so this stays a
// drop-in for the production path and nothing downstream needs to become async.
function computeScoredRowsV3() {
  const rows = products.map((product) => {
    const v3 = v3ScoreByUid.get(product.uid) || null;
    const score = v3 ? v3.score : -100;
    return {
      product,
      score,
      // V3 has no uplift ladder, so the evidence IS the score. Keeping the field equal
      // means the existing "calibrated above evidence" copy correctly never fires.
      evidenceScore: score,
      // V3's own components. The doctor-anchor, calibrated and type-prior layers are
      // deliberately absent: none of them contributes to a V3 score.
      featureScores: {
        concern: v3 ? v3.concernEvidence : 0,
        skin: v3 ? v3.skinFit : 0,
        ingredient: v3 ? v3.ingredientSuitability : 0,
        context: v3 ? v3.productContext : 0,
      },
      v3,
      // V3 ships NO ranking implementation. The old 55%-doctor-anchor rank fusion is
      // deliberately not reused, so there is no ranking score to report.
      rankingScore: null,
    };
  });
...
function computeScoredRows() {
  if (V3_MODE) return computeScoredRowsV3();
  const rows = products.map((product) => {
    const featureScores = {
...
function detailExplanation(row) {
  ...
  return `
    ...
    ${
      V3_MODE
        ? `<p>
      This is an Experimental V3 score: the strongest relevant ingredient evidence for this profile, attenuated by ingredient tier, reduced by any negative evidence, and capped by product context and safety. Neutral ingredients are excluded and <strong>no doctor calibration is applied</strong>. The ceiling is 90.
      Strongest supporting ingredient: <strong>${escapeHtml(row.v3?.strongestPositiveIngredient || "none recorded")}</strong>. Evidence strength: ${escapeHtml(row.v3?.evidenceStrength || "n/a")}, from ${row.v3?.strongPositiveCount ?? 0} well-suited active(s), with ${confidence} confidence.
    </p>`
        : `<p>
...
// Same five tiles and the same grid in both modes - only the labels and the values
// behind them differ, so the detail layout is unchanged.
function layerTiles(row) {
  if (V3_MODE) {
    const v3 = row.v3 || {};
    return [
      layerTile("Concern evidence", row.featureScores.concern),
      layerTile("Skin fit", row.featureScores.skin),
      layerTile("Ingredient suitability", row.featureScores.ingredient),
      layerTile("Product context", row.featureScores.context),
      layerTile("Safety cap", v3.safetyCap),
    ].join("");
  }
  return [
...
function renderAll() {
  if (!dataset) return;
  if (V3_MODE && !v3ScoreByUid) return; // first V3 response has not arrived yet
...
// V3 scores live on the server, so a profile change has to re-score before rendering.
// Filters and search do not change the profile, but re-scoring costs well under a second
// and keeping one path avoids touching any of the existing event handlers.
async function renderAllV3() {
  try {
    await fetchV3Scores();
  } catch (error) {
    console.error("[V3] scoring request failed", error);
    return;
  }
  renderAll();
}
...
function scheduleRender() {
  ...
  renderTimer = window.setTimeout(V3_MODE ? renderAllV3 : renderAll, 40);
}
...
async function init() {
  ...
  renderFilters();
  bindEvents();
  if (V3_MODE) await fetchV3Scores();
  renderAll();
}
